...
pub fn encode(docs: &[u32]) -> Vec<u8> {
    ...
    let mut compressed = vec![0u8; 8 * docs.len() + 32];
...
fn main() {
    ...
    let data = encode(postings);
    println!("encoded postings: {:?}", data);

    let mut output_buf = [0u32;1000];
    decode(&data, &mut output_buf, data.len() as u32);  
    let mut output = vec![];
    output.extend_from_slice(&output_buf[..(data.len())]);
